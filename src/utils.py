...
# Определим полный путь к файлу json
...
def read_json_files(json_path):
    try:
        with open(json_path, 'r', encoding='utf-8') as f:
            ...
            print(type(list_dict_transactions))
...
def sum_transactions(transactions: List[Dict[str, Any]]) -> float:
    ...
    for transaction in transactions:
        if transaction.get("operationAmount", {}).get("currency", {}).get("code") == 'RUB':
            ...
            total += amount
        #elif transaction.get("operationAmount", {}).get("currency", {}).get("code") != 'RUB':
...
if __name__ == '__main__':
    ...
    if list_dict_transactions is not None:
        ...
        total_sum = sum_transactions(list_dict_transactions)
    print(total_sum)
...
# convert_transaction_to_rub(transaction: dict) -> float:
"""
